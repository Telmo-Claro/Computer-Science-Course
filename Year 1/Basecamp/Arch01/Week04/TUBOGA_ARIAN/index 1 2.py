...
def pay_your_tax(player):
    ...
    time.sleep(3)
    print("Let me take 10 percent of your forins for taxes.")
    tax = round(player["balance"] * .1, 2)
    time.sleep(3)
...
def field_check(player):
    ...
    if index in [16, 34]:
        ...
        time.sleep(2)
        print(f"\nYou're getting 200 florins for getting on the START again")
        player["balance"] += 200
        time.sleep(2)
    elif index in [2, 20]:
        pay_your_tax(player)
...
def has_all_parts(player):
    for count in player["inventory"].values():
        if count == 0:
            return False
    return True
...
def famous_person(player):
    ...
    if has_all_parts(player):
        selling_price = random.randint(1050, 1500)
        while True:
...
def shop(player):
    ...
    prices = prices_shop["shop1"] if player["index"] == 9 else prices_shop["shop2"]
    ...
    if player["balance"] >= lowest_price:
        while shop_open:
            ...
            print(lines)
            if has_all_parts(player):
                sell_price = random.randint(950, 1200)
                buy_or_sell = input(f"\nDo you want to sell a computer for {sell_price}?\n Or would you like to buy something? Buy/Sell: ")
                if buy_or_sell.lower() == "sell":
                    for item in player["inventory"].values():
                        item -= 1
                    player["balance"] += sell_price
                    time.sleep(2)
                    print(f"\nYou successfully sold your computer for {sell_price} florins.")
                    offer = input("\nWould you also like to buy something? Y/N: ")
            else:
            ...
            if offer.lower() in ["y", "yes"]:
                while True:
                    ...
                    if offer1.lower() not in items:
                        ...
                        if player["balance"] >= prices[offer1]:
                            player["balance"] -= prices[offer1]
                            ...
                            while True:
                                leave_shop = input("\nWould you still like to buy something? Y/N: ")
                                if leave_shop.lower() in ["y", "yes"]:
                                    break
                                elif leave_shop.lower() in ["n", "no"]:
                                    print("\nThanks for shopping with us!")
                                    shop_open = False
                                    break
                                else:
...
def simulate_round():
    ...
    if p1["name"][-1].lower() == "s" or p1["name"][-1].lower() == "x":
        print(f"\n{p1['name']}' turn ({p1['sign']})")
    else:
        print(f"\n{p1['name']}'s turn ({p1['sign']})")
    ...
    if p2["name"][-1].lower() == "s" or p2["name"][-1].lower() == "x":
        print(f"\n{p2['name']}' turn ({p2['sign']})")
    else:
        print(f"\n{p2['name']}'s turn ({p2['sign']})")
...
while game_running:
    ...
    time.sleep(1)
    print(f"\n{p1['name']} is X and {p2['name']} is O on the playboard.")
    time.sleep(3)
