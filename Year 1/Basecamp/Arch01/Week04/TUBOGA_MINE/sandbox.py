...
while game_running:
    ...
    while True:
        def simulate_round():
            global p1_index, p2_index, p1_balance, p2_balance
            ...
            p1_index = update_position(p1_index, d1 + d2)
            p1_balance += d1 * d2
            print(f"Balance {p1}: {p1_balance}")

            if p1_balance >= 5000:
                return

            # Declare indexes for players
            if p1_index > len(indexes):
                index = p1_index
                p1_index = index % len(indexes)
            row_index_player1, column_index_player1 = indexes[p1_index]
            row_index_player2, column_index_player2 = indexes[p2_index]
            ...
            p2_index = update_position(p2_index, d3 + d4)
            p2_balance += d3 * d4
            print(f"Balance {p2}: {p2_balance}")
            if p2_balance >= 5000:
                return
            if p2_index > len(indexes):
                index = p2_index
                p2_index = index % len(indexes)
            row_index_player2, column_index_player2 = indexes[p2_index]
