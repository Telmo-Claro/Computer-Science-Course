...
class CarParkingMachine:
    def __init__(self, id, capacity=10, hourly_rate=2.50, parked_cars={}):
        ...
        self.json_path = self.id + "_state.json"
        self.json_data = []
        try:
            with open("carparklog.txt", "r") as file:
                for lines in file.readlines():
                    if f"cpm_name={self.id}" in lines:
    ...
    def check_in(self, license_plate, check_in=datetime.now()):
        ...
        check_in_string = check_in.strftime("%m-%d-%Y %H:%M:%S")
        can_check_in = True
        if len(self.parked_cars) >= self.capacity:
            return False
        with open("carparklog.txt", "r") as file:
            for line in file.readlines():
                if license_plate in line and "action=check-in" in line:
                    can_check_in = False
                if license_plate in line and "action=check-out" in line:
                    can_check_in = True
        if can_check_in:
            self.parked_cars[license_plate] = ParkedCar(license_plate, check_in)
            self.car_parking_logger.log_check_in(license_plate, check_in)
            new_json_check_in = {"license_plate": license_plate, "check_in": check_in_string}
            try:
                with open(self.json_path, "r") as file:
                    try:
                        data = json.load(file)
                        data.append(new_json_check_in)
                    except:
                        data = [new_json_check_in]
